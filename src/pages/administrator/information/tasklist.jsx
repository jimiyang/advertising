  ...
  settleEvent = (item) => {
    ...
    settleCampaign(params).then(rs => {
      ...
      if(!rs.data[0].success) {
        color = 'red-color'
        msg = `结算任务订单失败 ${rs.data[0].data}`
      } else {
        color = 'green-color'
        msg = `结算任务订单成功 ${rs.data[0].data}`
      }
  ...
  batchSettleEvent = () => {
    ...
    settleCampaign(params).then(rs => {
      ...
      rs.data.map((item, index) => {
        if (!item.success) {
          arr.push(<p className="red-color">结算任务订单失败 公众号：{item.data}}</p>)
        } else {
          arr.push(<p key={index} className="green-color">结算任务订单成功  公众号：{item.data}</p>)
        }
      })
            ...
            <div className="opeartion-items">
              {record.missionStatus === 13 ?
                <Popconfirm
                  ...
                  cancelText="否"
                >
                  <span className="blue-color">结算</span> 
                </Popconfirm>
                : null
              }
              <Link to={{pathname: '/main/taskdetail', state: {id: record.missionId}}} className="blue-color">详情</Link>
            </div>
          </div>
        )
      }
