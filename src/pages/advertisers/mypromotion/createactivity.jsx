  ...
  render() {
    ...
    const maxValue = (rule, value, callback) => {
      if (Number(value) > Number(availableBalance)) {
        callback('可用余额不足，请进行充值')
      } // 必须总是返回一个 callback，否则 validateFields 无法响应
      let reg = /^([1-9][0-9]*){1,3}|(^[0-9][.][0-9]{1}$)|(^[0-9][.][0-9]{2}$)$/
      if (!reg.test(value)) {
        callback('请输入大于0的整数或小数(保留后两位)')
      }
      callback() 
    }
    return(
        ...
        <div className={style.createBlocks}>
            ...
            <Form {...formItemLayout} onSubmit={this.createEvent} className={style.form} name="form" id="form">
                ...
                <Form.Item label="活动周期" {...tailFormItemLayout}>
                  {
                    getFieldDecorator(
                      'date',
                      {
                        ...
                        className="mr10"
                        format="YYYY-MM-DD"
                        value={form.dateStart === null || form.dateStart === '' ? null : moment(form.dateStart)}
                        placeholder="请输入开始时间"
                      ...
                      />至<DatePicker
                        ...
                        className="ml10"
                        format="YYYY-MM-DD"
                        value={form.dateEnd === null || form.dateEnd === '' ? null : moment(form.dateEnd)}
                        placeholder="请输入结束时间"
                    ...
                    <Form.Item label="阅读单价" {...tailFormItemLayout}>
                      {
                        ...
                        )(<div><Input value={form.unitPrice} className={style.ipttxt} onChange={this.changeFormEvent.bind(this, 'unitPrice')} />元/次阅读</div>)
                      }
                    </Form.Item>
                  </li>
                  <li style={{width: '100%'}}>
                    <Form.Item label="活动预算" {...tailFormItemLayout}>
                      {
                        getFieldDecorator(
                          'postAmtTotal',
                          {
                            initialValue: form.postAmtTotal || null,
                            rules: [
                ...
                <Form.Item>
                  ...
                  <Button className="ml30" onClick={this.handleReset.bind(this)}>取消活动</Button>
                </Form.Item>
            </Form>
        </div>
      </div>
